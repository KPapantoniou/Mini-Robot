...
pair<double,double> Robot::calculate_asynchronous_movement(double t, double dt) {
    ...
    double angle_A = atan((l/2) / (w/2));     
    double angle_B = atan((-l/2) / (w/2));
    double angle_C = M_PI; 
    ...
    double vel_A_x = vel_x - omega * sqrt(pow(l/2,2) + pow(w/2,2)) * sin(angle_A);
    double vel_A_y = vel_y + omega * sqrt(pow(l/2,2) + pow(w/2,2)) * cos(angle_A);
    
    double vel_B_x = vel_x - omega * sqrt(pow(l/2,2) + pow(w/2,2)) * sin(angle_B);
    double vel_B_y = vel_y + omega * sqrt(pow(l/2,2) + pow(w/2,2)) * cos(angle_B);
    ...
    current_theta = theta_displacement;
    acc_x = linear_acceleration_calculator(force_x, mass);
    acc_y = linear_acceleration_calculator(force_y, mass);

    vel_x = velocity_calculator(acc_x, vel_x, dt);
    vel_y = velocity_calculator(acc_y, vel_y, dt);
    global_vel_x = vel_x * cos(current_theta) - vel_y * sin(current_theta);
    global_vel_y = vel_x * sin(current_theta) + vel_y * cos(current_theta);
    
    pos_x = position_calculator(global_vel_x, pos_x, 0, dt); 
    pos_y = position_calculator(global_vel_y, pos_y, 0, dt);
...
double Robot::inertia() const{
     double inertia_mass = mass*pow(l,2)/2;
     return inertia_mass;
}
